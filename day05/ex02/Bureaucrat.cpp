...
void                            Bureaucrat::signForm(Form & form) {
    if (form.getMinSign() >= this->getGrade())
        std::cout << this->getName() << " signs " << form.getName() << std::endl;
    else
        std::cout << this->getName() << " cannot sign " << form.getName() << " because he's grade is too low." << std::endl;
    return ;
}

bool                            Bureaucrat::executeForm(Form & form) {
    if (form.getMinEx() >= this->getGrade() && form.isSigned()) {
